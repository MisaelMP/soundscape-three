...
export function useSoundInput() {
  ...
  const initAudio = async () => {
    try {
      ...
      const updateSoundData = () => {
        ...
        const amplitude = dataArray.reduce((acc, val) => acc + Math.abs(val), 0) / bufferLength
        const frequency = getDominantFrequency(dataArray, audioContext.value?.sampleRate || 44100)
  ...
  const getDominantFrequency = (data: Float32Array, sampleRate: number): number => {
